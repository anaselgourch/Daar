...
export default function BestSellers({ products }: { products: any[] }) {
  ...
  return (
    // Reduced padding-top on mobile (pt-5) to bring the section up
    <section className="bg-white pt-3 md:pt-8 pb-8 px-6 md:px-16 lg:px-24 relative">
      <div className="flex justify-between items-end pb-2 mb-4">
        <h2 className="text-[18px] font-bold uppercase tracking-widest text-[#221E1F] font-montserrat">
          Best Sellers
        </h2>
        <Link href="/shop" className="text-[13px] tracking-normal text-[#221E1F] font-[500] underline">
          Shop All
        </Link>
      </div>
      ...
      <div className="relative group">
        <button 
          onClick={() => scroll("left")}
          className="hidden md:flex absolute  top-[40%] z-30 bg-[#E7E5E2] backdrop-blur-md w-8 h-8 rounded-full items-center justify-center shadow-sm cursor-pointer transition-opacity duration-300 opacity-0 group-hover:opacity-100 text-[#221E1F] hover:bg-black hover:text-white"
        >
        ...
        <button 
          ...
          className="hidden md:flex absolute right-[-2px] top-[40%] z-30 bg-[#E7E5E2] backdrop-blur-md w-8 h-8 rounded-full items-center justify-center shadow-sm cursor-pointer transition-opacity duration-300 opacity-0 group-hover:opacity-100 text-[#221E1F] hover:bg-black hover:text-white"
        >
...
function ProductCard({ product }: { product: any }) {
  ...
  return (
    <div 
      className={`product-card min-w-[85%] md:min-w-[320px] lg:min-w-[24%] snap-center md:snap-start flex flex-col mx-auto cursor-pointer ${montserrat.className}`}
    ...
    >
      <div className="relative aspect-[4/5] bg-[#F3F3F3] overflow-hidden mb-[-4px]">
        {product.tag && (
          <div className="absolute top-3 left-3 bg-white px-2 py-1 z-10">
            <span className="text-[8px] font-bold tracking-[0.1em] text-black uppercase">{product.tag}</span>
          </div>
        )}
      ...
      <div className="mt-4 flex flex-col items-center md:items-start text-center md:text-left">
        ...
        <p className="text-[12px] text-gray-500 mt-0.5 line-clamp-1 font-medium">{product.description}</p>
